...
def get_bearing_cd(origin, destination):
    bearing = np.rad2deg(np.arctan2(
        destination[1]-origin[1], destination[0]-origin[0])) * 100
    if (bearing < 0):
        bearing += 36000.0
    return wrap_180_cd(bearing)


def get_bearing_rad(destination, origin=(0,0)):
...
def get_bearing_deg(destination, origin=(0,0)):
    bearing = np.rad2de(np.arctan2(
...
class SqrtController():
    ...
    def __call__(self, error, p, second_ord_lim, dt=None):
        ...
        if (error_length > linear_dist):
            first_order_scale = safe_sqrt(
                2.0 * second_ord_lim * (error_length - (linear_dist * 0.5))) / error_length
            output = [error[0] * first_order_scale,
                      error[1] * first_order_scale, error[2]]
        else:
            output = [error[0] * p, error[1] * p, error[2]]
    ...
    def __call__(self, error, p, second_ord_lim, dt=None):
        ...
        if ((second_ord_lim < 0) or (not np.all(second_ord_lim))):
            ...
            if (error > linear_dist):
                correction_rate = safe_sqrt(
                    2.0 * second_ord_lim * (error - (linear_dist / 2.0)))
            elif (error < -linear_dist):
                correction_rate = - \
                    safe_sqrt(2.0 * second_ord_lim *
                              (-error - (linear_dist / 2.0)))
            else:
                correction_rate = error * p

        if np.all(dt):
            # this ensures we do not get small oscillations by over shooting the error correction in the last time step.
            output = np.clip(correction_rate, -abs(error) /
                             dt, abs(error) / dt)
